...
export const showReviews = async (req, res) => {
    try {
        ...
        const reviewWithUserDetails = await Promise.all(showReviews.map(async (review) => {
            const user = await User.findById(review.user); // Find user details
            return {
                ...review._doc,
                user: {
                    id: user._id,
        ...
        return res.status(200).json({ message: reviewWithUserDetails, success: true })
